...
class TestStructures(unittest.TestCase):
    ...
    def test_target_specific_features(self):
        ...
        self.assertEqual(target.mean_dose(), 60.0)
        self.assertEqual(target.max_dose(), 60.0)
        self.assertEqual(target.min_dose(), 60.0)

        # Test DVH computation
        bins, volumes = target.dvh(max_dose=80, step_size=1.0)
        self.assertIsNotNone(bins)
        self.assertIsNotNone(volumes)
        # At 0 Gy, 100% of volume should be above
        self.assertEqual(volumes[0], 100.0)
    ...
    def test_comprehensive_workflow(self):
        ...
        # Test target-specific metrics (using available methods)
        prescription_dose = 60.0  # Gy
        target_mean_dose = target.mean_dose()
        self.assertIsNotNone(target_mean_dose)
        if target_mean_dose is not None:
            self.assertGreater(target_mean_dose, 0)
