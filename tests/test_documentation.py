...
def test_notebook_execution(notebook_path, notebook_name, executor):
    """
...
def test_notebook_structure():
    ...
    for notebook_path in NOTEBOOKS_DIR.glob("*.ipynb"):
        ...
        if nb.cells[0].cell_type == 'markdown':
            ...
            assert first_cell.startswith('#'), f"{notebook_path.name} first cell is not a title"
        ...
        for cell in nb.cells:
            if cell.cell_type == 'code':
                ...
                if 'snapshot_download' in source and 'repo_id=' in source:
                    assert correct_dataset in source, \
