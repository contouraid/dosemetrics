"""
Tests for dosemetrics.metrics.scores module.

Tests dose scoring and DVH scoring functions.
"""
...
class TestScores(unittest.TestCase):
    """Test dose and DVH scoring functions."""

    def test_dose_score(self):
        """Test dose scoring functionality."""
        logger.info("Testing dose score ...")
        zero_dose = np.zeros((10, 10, 10))

        zero_score = dosemetrics.dose_score(zero_dose, zero_dose)
        self.assertTrue(zero_score == 0.0)

        single_voxel_struct = np.zeros((10, 10, 10))
        single_voxel_struct[5, 5, 5] = 1
        non_zero_score = dosemetrics.dose_score(zero_dose, single_voxel_struct)
        self.assertTrue(non_zero_score > 0.0)

        no_difference_score = dosemetrics.dose_score(
            single_voxel_struct, single_voxel_struct
        )
        self.assertTrue(no_difference_score == 0.0)

        commutative_score = dosemetrics.dose_score(single_voxel_struct, zero_dose)
        ...
        zero_mask_score = dosemetrics.dose_score(
        ...
        larger_score = dosemetrics.dose_score(large_single_voxel_struct, zero_dose)
        self.assertTrue(larger_score > non_zero_score)
        logger.info("... completed testing dose score.")

    def test_dvh_score(self):
        """Test DVH scoring functionality."""
        logger.info("Testing DVH score ...")
        zero_dose = np.zeros((10, 10, 10))
        single_voxel_struct = np.zeros((10, 10, 10))
        single_voxel_struct[5, 5, 5] = 1

        zero_score = dosemetrics.dvh_score(
            zero_dose, single_voxel_struct, "OAR", (1, 1, 1)
        )
        ...
        non_zero_score = dosemetrics.dvh_score(zero_dose, single_voxel_struct, "Target")
