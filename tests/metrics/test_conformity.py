...
class TestRTOGConformityIndex:
    """Test RTOG Conformity Index: V_Rx / V_target."""

    def test_rtog_ci_ideal_plan(self, ideal_dose, ideal_target):
        # Dose region == target region → RTOG CI ≈ 1.0
        rtog_ci = conformity.compute_rtog_conformity_index(
            ideal_dose, ideal_target, 60.0
        )
        assert rtog_ci == pytest.approx(1.0, abs=0.01)

    def test_rtog_ci_spillage_greater_than_one(self, spillage_dose, ideal_target):
        # Isodose volume > target → RTOG CI > 1
        rtog_ci = conformity.compute_rtog_conformity_index(
            spillage_dose, ideal_target, 60.0
        )
        assert rtog_ci > 1.0

    def test_rtog_ci_undercoverage_less_than_one(
        self, undercoverage_dose, ideal_target
    ):
        # Only half of target at Rx dose → V_Rx < V_target → RTOG CI < 1
        rtog_ci = conformity.compute_rtog_conformity_index(
            undercoverage_dose, ideal_target, 60.0
        )
        assert rtog_ci < 1.0

    def test_rtog_ci_returns_float(self, ideal_dose, ideal_target):
        rtog_ci = conformity.compute_rtog_conformity_index(
            ideal_dose, ideal_target, 60.0
        )
        assert isinstance(rtog_ci, float)

    def test_rtog_ci_empty_target_returns_nan(self, ideal_dose):
        empty = Target(
            "Empty",
            np.zeros((50, 50, 30), dtype=bool),
            (2.0, 2.0, 3.0),
        ...
        rtog_ci = conformity.compute_rtog_conformity_index(ideal_dose, empty, 60.0)
        assert np.isnan(rtog_ci)

    def test_rtog_ci_differs_from_icru_ci(self, spillage_dose, ideal_target):
        # RTOG CI = V_Rx / V_target; ICRU CI = V_target_rx / V_rx — different metrics
        rtog = conformity.compute_rtog_conformity_index(
            spillage_dose, ideal_target, 60.0
        )
        icru = conformity.compute_conformity_index(spillage_dose, ideal_target, 60.0)
        # With spillage: RTOG CI > 1, ICRU CI < 1
        assert rtog > 1.0
        assert icru < 1.0


class TestPrescriptionMAE:
    """Test Prescription MAE: mean |dose - prescription| within target."""

    def test_perfect_match_is_zero(self, ideal_dose, ideal_target):
        # All voxels at exactly 60 Gy = prescription → MAE = 0
        mae = conformity.compute_prescription_mae(ideal_dose, ideal_target, 60.0)
        assert mae == pytest.approx(0.0, abs=1e-6)

    def test_underdosing_gives_positive_mae(self, undercoverage_dose, ideal_target):
        mae = conformity.compute_prescription_mae(
            undercoverage_dose, ideal_target, 60.0
        )
        assert mae > 0.0

    def test_known_uniform_underdosing(self):
        # All voxels at 55 Gy, prescription 60 Gy → MAE = 5.0
        dose_array = np.zeros((30, 30, 20))
        dose_array[10:20, 10:20, 5:15] = 55.0
        dose = Dose(dose_array, (2.0, 2.0, 3.0), (0.0, 0.0, 0.0))
        mask = np.zeros((30, 30, 20), dtype=bool)
        mask[10:20, 10:20, 5:15] = True
        target = Target("PTV", mask, (2.0, 2.0, 3.0), (0.0, 0.0, 0.0))

        mae = conformity.compute_prescription_mae(dose, target, 60.0)
        assert mae == pytest.approx(5.0, abs=0.01)

    def test_returns_float(self, ideal_dose, ideal_target):
        mae = conformity.compute_prescription_mae(ideal_dose, ideal_target, 60.0)
        assert isinstance(mae, float)

    def test_empty_target_returns_nan(self, ideal_dose):
        empty = Target(
            "Empty",
            np.zeros((50, 50, 30), dtype=bool),
            (2.0, 2.0, 3.0),
        ...
        mae = conformity.compute_prescription_mae(ideal_dose, empty, 60.0)
        assert np.isnan(mae)
