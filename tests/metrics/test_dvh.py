...
class TestComputeDVH:
    ...
    def test_compute_dvh_custom_params(self, sample_dose, sample_structure):
        ...
        dose_bins, volumes = dvh.compute_dvh(
            sample_dose, sample_structure, max_dose=40.0, step_size=1.0
        )
    ...
    def test_compute_dvh_uniform_dose(self, uniform_dose, sample_structure):
        ...
        dose_bins, volumes = dvh.compute_dvh(
            uniform_dose, sample_structure, max_dose=51.0
        )
...
class TestDoseAtVolume:
    ...
    def test_dose_at_volume_bounds(self, sample_dose, sample_structure):
        ...
        d100 = dvh.compute_dose_at_volume(
            sample_dose, sample_structure, 100
        )  # Min dose
...
class TestEquivalentUniformDose:
    ...
    def test_eud_basic(self, sample_dose, sample_structure):
        ...
        eud = dvh.compute_equivalent_uniform_dose(
            sample_dose, sample_structure, a_parameter=-10
        )
    ...
    def test_eud_uniform_dose(self, uniform_dose, sample_structure):
        ...
        eud = dvh.compute_equivalent_uniform_dose(
            uniform_dose, sample_structure, a_parameter=-10
        )
    ...
    def test_eud_parameter_effect(self, sample_dose, sample_structure):
        ...
        eud_neg = dvh.compute_equivalent_uniform_dose(
            sample_dose, sample_structure, a_parameter=-10
        )
        eud_pos = dvh.compute_equivalent_uniform_dose(
            sample_dose, sample_structure, a_parameter=10
        )
...
class TestDVHTable:
    ...
    def test_create_dvh_table(self, sample_dose, hf_data_path):
        ...
        # Create DVH table
        df = dvh.create_dvh_table(
            dose, structures, structure_names=structures.structure_names[:3]
        )
...
class TestDVHAUC:
    """Test DVH Area Under Curve."""

    @pytest.fixture
    def uniform_dose_60(self):
        dose_array = np.ones((30, 30, 20)) * 60.0
        return Dose(dose_array, (2.0, 2.0, 3.0), (0.0, 0.0, 0.0))

    @pytest.fixture
    def gradient_dose(self):
    ...
    def full_structure(self):
        mask = np.ones((30, 30, 20), dtype=bool)
        return Target("Full", mask, (2.0, 2.0, 3.0), (0.0, 0.0, 0.0))

    @pytest.fixture
    def partial_structure(self):
        mask = np.zeros((30, 30, 20), dtype=bool)
        mask[10:20, 10:20, 5:15] = True
        return Target("Partial", mask, (2.0, 2.0, 3.0), (0.0, 0.0, 0.0))

    def test_uniform_dose_normalized_is_one(self, uniform_dose_60, full_structure):
        auc = dvh.compute_dvh_auc(uniform_dose_60, full_structure, normalize=True)
        assert auc == pytest.approx(1.0, abs=0.05)

    def test_normalized_auc_in_range(self, gradient_dose, full_structure):
        auc = dvh.compute_dvh_auc(gradient_dose, full_structure, normalize=True)
        assert 0.0 <= auc <= 1.0

    def test_unnormalized_auc_positive(self, gradient_dose, partial_structure):
        auc = dvh.compute_dvh_auc(gradient_dose, partial_structure, normalize=False)
        assert auc > 0.0

    def test_higher_dose_gives_higher_auc(self, full_structure):
        low_dose = Dose(np.ones((30, 30, 20)) * 20.0, (2.0, 2.0, 3.0), (0.0, 0.0, 0.0))
        high_dose = Dose(np.ones((30, 30, 20)) * 60.0, (2.0, 2.0, 3.0), (0.0, 0.0, 0.0))
        auc_low = dvh.compute_dvh_auc(low_dose, full_structure, normalize=False)
        auc_high = dvh.compute_dvh_auc(high_dose, full_structure, normalize=False)
        assert auc_high > auc_low

    def test_empty_structure_returns_zero(self, gradient_dose):
        empty = Target(
            "Empty",
            np.zeros((30, 30, 20), dtype=bool),
            (2.0, 2.0, 3.0),
        ...
        auc = dvh.compute_dvh_auc(gradient_dose, empty)
        assert auc == 0.0

    def test_fixed_dose_range(self, gradient_dose, partial_structure):
        auc1 = dvh.compute_dvh_auc(
            gradient_dose, partial_structure, dose_range=(0.0, 60.0)
        )
        auc2 = dvh.compute_dvh_auc(
            gradient_dose, partial_structure, dose_range=(0.0, 60.0)
        )
