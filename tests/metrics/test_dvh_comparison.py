"""
Tests for consolidated dosemetrics.metrics.dvh operations.

Tests DVH comparisons and multi-plan summaries in the core DVH module.
"""
...
class TestWassersteinDistance:
    ...
    def test_wasserstein_identical(self, dose1, test_structure):
        ...
        distance = dvh.compare_dvh_wasserstein(dose1, dose1, test_structure)
        assert distance >= 0
    ...
    def test_wasserstein_similar(self, dose1, dose2_similar, test_structure):
        ...
        distance = dvh.compare_dvh_wasserstein(dose1, dose2_similar, test_structure)
        assert distance >= 0
    ...
    def test_wasserstein_different(self, dose1, dose3_different, test_structure):
        ...
        distance = dvh.compare_dvh_wasserstein(dose1, dose3_different, test_structure)
        assert distance > 0
        assert isinstance(distance, float)
...
class TestAreaBetweenCurves:
    ...
    def test_area_identical_l1(self, dose1, test_structure):
        ...
        area = dvh.compare_dvh_area(dose1, dose1, test_structure, norm="L1")
        assert area >= 0
    ...
    def test_area_identical_l2(self, dose1, test_structure):
        ...
        area = dvh.compare_dvh_area(dose1, dose1, test_structure, norm="L2")
        assert area >= 0
        assert area < 1.0  # Should be very small

    def test_area_different(self, dose1, dose3_different, test_structure):
        """Test area is positive for different doses."""
        area = dvh.compare_dvh_area(dose1, dose3_different, test_structure)
    ...
    def test_area_invalid_norm(self, dose1, dose2_similar, test_structure):
        ...
        with pytest.raises(ValueError, match="norm"):
            dvh.compare_dvh_area(dose1, dose2_similar, test_structure, norm="L3")
...
class TestChiSquareTest:
    ...
    def test_chi_square_similar(self, dose1, dose2_similar, test_structure):
        ...
        chi2_stat, p_value = dvh.compare_dvh_chi_square(
...
class TestKSTest:
    ...
    def test_ks_identical(self, dose1, test_structure):
        ...
        ks_stat, p_value = dvh.compare_dvh_ks(dose1, dose1, test_structure)
    ...
    def test_ks_similar(self, dose1, dose2_similar, test_structure):
        ...
        ks_stat, p_value = dvh.compare_dvh_ks(dose1, dose2_similar, test_structure)
    ...
    def test_ks_different(self, dose1, dose3_different, test_structure):
        ...
        ks_stat, p_value = dvh.compare_dvh_ks(dose1, dose3_different, test_structure)
...
class TestConfidenceInterval:
    ...
    def test_confidence_interval_single(self, dose1, test_structure):
        ...
        dose_bins, mean_dvh, ci_lower, ci_upper = dvh.compute_dvh_confidence_interval(
            [dose1], test_structure
        )
    ...
    def test_confidence_interval_empty(self, test_structure):
        ...
        with pytest.raises(ValueError):
            dvh.compute_dvh_confidence_interval([], test_structure)
...
class TestBandwidth:
    ...
    def test_bandwidth_single(self, dose1, test_structure):
        ...
        _, bandwidth = dvh.compute_dvh_bandwidth([dose1], test_structure)

        assert len(bandwidth) > 0
        # Single dose should have zero bandwidth
        assert np.allclose(bandwidth, 0)

    def test_bandwidth_multiple(
        self, dose1, dose2_similar, dose3_different, test_structure
    ):
        """Test bandwidth with multiple doses."""
        doses = [dose1, dose2_similar, dose3_different]
        _, bandwidth = dvh.compute_dvh_bandwidth(doses, test_structure)
    ...
    def test_bandwidth_identical(self, dose1, test_structure):
        ...
        _, bandwidth = dvh.compute_dvh_bandwidth([dose1, dose1, dose1], test_structure)
    ...
    def test_bandwidth_empty(self, test_structure):
        """Test error with empty dose list."""
        with pytest.raises(ValueError):
            dvh.compute_dvh_bandwidth([], test_structure)


class TestIntegration:
    """Integration tests for consolidated DVH metrics."""

    def test_multiple_metrics(self, dose1, dose2_similar, test_structure):
        """Test computing multiple metrics together."""
        # Wasserstein distance
        wd = dvh.compare_dvh_wasserstein(dose1, dose2_similar, test_structure)

        # Area between curves
        area = dvh.compare_dvh_area(dose1, dose2_similar, test_structure)
