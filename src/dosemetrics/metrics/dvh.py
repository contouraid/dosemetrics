...
# =============================================================================
# BACKWARD COMPATIBILITY: Legacy array-based functions (deprecated)
# These are kept for compatibility with utils.plot module
# TODO: Remove after updating plot.py to use new API
# =============================================================================

def _compute_dvh_arrays(
    ...
    """
    Legacy array-based DVH computation (deprecated).
    
    This function is kept for backward compatibility with plot.py.
    New code should use compute_dvh(dose, structure) instead.
    """
    # Extract dose values in structure
    dose_in_structure = dose_array[structure_mask > 0]
    ...
    # Calculate volume at each dose
    for i, dose_level in enumerate(bins):
        volumes[i] = np.sum(dose_in_structure >= dose_level) / len(dose_in_structure) * 100
    
    return bins, volumes


# Alias for compatibility
compute_dvh_legacy = _compute_dvh_arrays
