...
def generate_dvh_family(dose_volume, structure_masks, constraints: pd.DataFrame, structure_of_interest: str):

    structure_mask = structure_masks[structure_of_interest]
    constraint_limit = constraints.loc[structure_of_interest, "Level"]
    fig, dsc_range = plot.dvh_family(dose_volume, structure_mask, constraint_limit, structure_of_interest)
...
def panel():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["🗃️Upload Data", "📊 View Dose Metrics", "🔍 Compute Compliance", "✅Evaluate Contour"])

    with tab1:
        st.markdown(f"## Step 1: Upload dose distribution volume and mask files")

        st.markdown("Upload the dose volume:")
        dose_file = st.file_uploader(f"Upload dose volume: (in .nii.gz)", type=['nii', 'gz'], key=0)
    ...
    with tab2:
        ...
        if step_1_complete:
            ...
            st.download_button(label="Download CSV", data=csv, file_name=f"dose_summary_df.csv", mime="text/csv",
                               key=999)
    ...
    with tab3:
        ...
        if step_2_complete:
            ...
            compliance_results = compliance.compute_mirage_compliance(dose, structure_mask)
            st.table(compliance_results)
            compliance_csv = compliance_results.to_csv(index=True)
            st.download_button(label="Download compliance CSV", data=compliance_csv, file_name="compliance.csv",
                               mime="text/csv", key=500)
    ...
    with tab4:
        ...
        if step_3_complete:
            ...
            option = st.pills("Choose structure:", tuple(structure_mask.keys()), selection_mode="single")
            st.divider()
